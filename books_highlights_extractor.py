...
class BooksHighlightExtractor:
    ...
    def show_single_book(self, book_path_or_title):
        """显示单个书籍的高亮和笔记"""
        books = self.get_books_info()
        ...
        for book in books:
            ...
            # 支持多种匹配方式
            if (str(asset_id) == book_path_or_title or 
                book_path_or_title.lower() in title.lower() or
                title == book_path_or_title):
                matched_book = book
                break
        
        if not matched_book:
            print(f"❌ 没有找到匹配的书籍: {book_path_or_title}")
            print("\n💡 提示: 可以使用以下方式之一:")
            print("   - 完整书名")
            print("   - 书名关键词")
            print("   - 书籍ID")
        ...
        # 提取书籍信息
        asset_id = matched_book[0] if len(matched_book) > 0 else None
        title = matched_book[1] if len(matched_book) > 1 else "未知书名"
        author = matched_book[2] if len(matched_book) > 2 else None
        genre = matched_book[3] if len(matched_book) > 3 else None
        publisher = matched_book[4] if len(matched_book) > 4 else None
        publish_date = matched_book[5] if len(matched_book) > 5 else None
        last_open_date = matched_book[6] if len(matched_book) > 6 else None
        ...
        for i, annotation in enumerate(annotations, 1):
            ...
            print(f"\n{i}. {color_emoji} 高亮 ({color_name})")
            print("-" * 40)
            
            # 显示高亮文本
            if selected_text:
                print(f"💡 高亮内容:")
                print(f"   {selected_text.strip()}")
            ...
            # 显示时间信息
            if creation_date:
                print(f"⏰ 创建时间: {self.format_timestamp(creation_date)}")

def main():
    parser = argparse.ArgumentParser(description='Mac Books 高亮摘录提取器')
    parser.add_argument('--list', action='store_true', help='列出所有书籍')
    parser.add_argument('--show-book', metavar='BOOK', help='显示单个书籍的高亮（支持书名/关键词/ID）')
    ...
    if args.list:
        ...
        extractor.show_single_book(args.show_book)
