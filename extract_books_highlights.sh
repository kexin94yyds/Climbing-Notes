...
case $choice in
    ...
    6)
        echo "📚 显示单个书籍的高亮"
        echo ""
        echo "💡 提示: 您可以输入以下任一种方式:"
        echo "   - 完整书名 (如: 七年就是一辈子)"
        echo "   - 书名关键词 (如: 七年)"
        echo "   - 书籍ID (先运行选项1查看)"
        echo ""
        read -p "请输入书名/关键词/ID: " book_input
        
        if [ -z "$book_input" ]; then
            echo "❌ 请输入有效的书名或ID"
        else
            echo "🔍 正在查找书籍: $book_input"
            python3 books_highlights_extractor.py --show-book "$book_input"
        fi
        ;;
    0)
        echo "👋 再见!"
